"""
BiplaneSimpsonClinical—双平面Simpson法LVEF计算
主要参数：
n_discs:辛普森圆盘数量（默认20）
cavity_label:LV腔体标签值（默认1）
annulus_strategy:"polar"或"wall_la"
band_frac:切片宽度系数（polar策略建议1.0；wall_la策略建议0.2）
min_band_points:切片最少像素数（默认3）
robust_width:True=95%-5%百分位宽度；False=max-min（默认False）
enforce_apex_zero:边界外直径置0（默认True）
keep_largest_cc:保留最大连通域（默认True，当前为pass-through）
"""
...
class BiplaneSimpsonClinical:
    ...
    def _find_ed_es_simple(self,area_px,view_name=""):
    ...
    def _find_ed_es_robust_video(self,area_px,view_name=""):
        area_px=np.asarray(area_px,dtype=float)
        n=len(area_px)
        if n == 0:
            return -1,-1
        ...
        if mad < 1e-6:
            ...
            if np.sum(valid) < max(3,n // 2):
                ...
                valid_idx=np.where(valid)[0]
                valid_vals=area_px[valid]
                #核心：在有效帧里，如果有两帧以上，对面积排序后取次小值（索引为1）
                if len(valid_vals) >= 2:
    ...
    def _find_ed_es(self,area_px,view_name=""):
        if self.ed_es_mode == "simple":
            return self._find_ed_es_simple(area_px,view_name)
        return self._find_ed_es_robust_video(area_px,view_name)
    ...
    def compute_ed_es_from_series(self,masks2,masks4,spacing2,spacing4):
        ...
        ed2,es2=self._find_ed_es(area2,view_name="2CH")
        ed4,es4=self._find_ed_es(area4,view_name="4CH")
    ...
    self,masks,spacing,method:str="singleplane",view_name:str="2CH"
        ...
        ed_i,es_i=self._find_ed_es(area_px,view_name=view_name)
