...
class LoginDialog(QDialog):
    def __init__(self,background_image_path=None,background_opacity=0.5):
        super().__init__()
        self.token: Optional[str]=None
        self.password_visible=False
        self.background_image_path=background_image_path
        self.background_opacity=background_opacity  #背景图片透明度
    ...
    def set_background_opacity(self,opacity):
        ...
        参数:
            opacity: 0.0-1.0 之间的值，0为完全透明，1为完全不透明
                    建议范围：0.3-0.7
        """
    ...
    def _build_ui(self):
        ...
        """)
        content_frame.setFixedSize(400,360)
        ...
        #标题
        title=QLabel("心脏超声智能分析系统")
        title.setFont(QFont("Microsoft YaHei",18,QFont.Bold))
        title.setAlignment(Qt.AlignCenter)
        title.setStyleSheet("color: #333333; margin-bottom: 35px; background: transparent;")
        ...
        #用户图标
        user_icon=QLabel()
        user_icon.setPixmap(self._create_user_icon())
        ...
        username_frame.setLayout(username_layout)
        ...
        #密码图标
        pwd_icon=QLabel()
        pwd_icon.setPixmap(self._create_password_icon())
        ...
        """)
        self.eye_button.setIcon(self._create_eye_icon(closed=True))
        ...
        #添加输入框到容器
        input_layout.addWidget(username_frame)
        input_layout.addWidget(password_frame)
        ...
        #添加到主布局（居中显示）
        main_layout.addWidget(content_frame,0,Qt.AlignCenter)
    ...
    def set_content_opacity(self,opacity):
        ...
        参数:
            opacity: 0.0-1.0 之间的值，0为完全透明，1为完全不透明
                    建议范围：0.85-0.95
        """
        opacity=max(0.0,min(1.0,opacity))  #限制在0-1之间
        #更新容器框的样式
        content_frame=self.findChild(QFrame,"contentFrame")
        if content_frame:
    ...
    def toggle_password_visibility(self):
        ...
        if self.password_visible:
            ...
            self.eye_button.setIcon(self._create_eye_icon(closed=False))
        else:
            self.password_input.setEchoMode(QLineEdit.Password)
            self.eye_button.setIcon(self._create_eye_icon(closed=True))
    ...
    def _create_user_icon(self):
        ...
        svg_data=b'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#666666">
    ...
    def _create_password_icon(self):
        """创建密码图标（SVG）"""
        from PyQt5.QtSvg import QSvgRenderer
        from PyQt5.QtGui import QPixmap,QPainter
    ...
    def _create_eye_icon(self,closed=True):
        """创建眼睛图标（闭眼/睁眼）"""
        from PyQt5.QtSvg import QSvgRenderer
        ...
        if closed:
            #闭眼图标
            svg_data=b'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#999999">
                <path d="M12 7c2.76 0 5 2.24 5 5 0 .65-.13 1.26-.36 1.83l2.92 2.92c1.51-1.26 2.7-2.89 3.43-4.75-1.73-4.39-6-7.5-11-7.5-1.4 0-2.74.25-3.98.7l2.16 2.16C10.74 7.13 11.35 7 12 7zM2 4.27l2.28 2.28.46.46C3.08 8.3 1.78 10.02 1 12c1.73 4.39 6 7.5 11 7.5 1.55 0 3.03-.3 4.38-.84l.42.42L19.73 22 21 20.73 3.27 3 2 4.27zM7.53 9.8l1.55 1.55c-.05.21-.08.43-.08.65 0 1.66 1.34 3 3 3 .22 0 .44-.03.65-.08l1.55 1.55c-.67.33-1.41.53-2.2.53-2.76 0-5-2.24-5-5 0-.79.2-1.53.53-2.2zm4.31-.78l3.15 3.15.02-.16c0-1.66-1.34-3-3-3l-.17.01z"/>
            </svg>'''
        else:
            #睁眼图标
            svg_data=b'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#0078D4">
